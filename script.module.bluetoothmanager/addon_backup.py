...
class MainClass(xbmcgui.WindowXMLDialog):
	...
	def onClick(self, controlID):
		...
		if controlID == CONNECT_BUTTON:
			...
			if bluetooth == OFF:
				xbmcgui.Dialog().ok(addonname, "$ADDON[script.module.bluetoothmanager 30004]")
			else:
				ctl = Bluetoothctl()
				run_command("pulseaudio -D")
				run_command("sudo hciconfig hci0 piscan")
				...
				p.start()
				xbmcgui.Dialog().yesno(addonname, "$ADDON[script.module.bluetoothmanager 30005]", yeslabel = "$ADDON[script.module.bluetoothmanager 30006]", nolabel = "$ADDON[script.module.bluetoothmanager 30007]")
				#ctl.pair_device()			
				p.terminate()
				run_command("sudo hciconfig hci0 noscan")
				#TODO connections and new connection in new Window !!
			del ctl
